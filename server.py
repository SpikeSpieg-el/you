...
def get_files_and_dirs(directory, sort_by=None, sort_order='asc', search_query=None, files_only=False):
    dirs = []
    files = []
                ...
                if entry.is_file():
                    ...
                    if not files_only:
                        dirs.append({
                            ...
                            'name': entry.name,
                            'path': os.path.relpath(entry.path, FILE_DIRECTORY),
                            'children': get_files_and_dirs(entry.path, sort_by, sort_order, search_query, files_only)
                        })
...
def contents():
    ...
    return jsonify(get_files_and_dirs(FILE_DIRECTORY, sort_by, sort_order, search_query, files_only))
